...
new_60 = (
          f'Data/60km/Rainfall/rainfall_hadukgrid'
          f'_uk_60km_day_20010401-20010430.nc'
        )   
...
with xr.open_dataset(new_60) as ds_new_60:
    ...
    ds_new_60 = ds_new_60.interpolate_na(dim='projection_x_coordinate',
                method='nearest')
    ds_new_60 = ds_new_60.interpolate_na(dim='projection_y_coordinate',
                method='nearest')
    ...
    with xr.open_dataset(new_12) as ds_new_12:
        ...
        data_slice_60['rainfall'] = data_slice_60['rainfall'].fillna(
                                    average_rainfall)
        ...
        interp_rainfall_linear = griddata((x_mesh_60.flatten(), 
                                           y_mesh_60.flatten()), 
...
# Calculate the max
vmax = max(np.nanmax(rainfall_data_60_original), 
            np.nanmax(interp_rainfall_linear), np.nanmax(rainfall_data_12))

# Input
plt.subplot(1, 3, 1)
plt.imshow(rainfall_data_60_original, cmap='Blues', origin='lower', 
...
plt.subplot(1, 3, 2)
plt.imshow(interp_rainfall_linear, cmap='Blues', origin='lower',
             aspect='auto', vmin=0, vmax=vmax)
plt.colorbar(label='Rainfall (mm)')
plt.title('Linear Interpolation')

# Plot true rainfall values at 12km resolution
plt.subplot(1, 3, 3)
plt.imshow(rainfall_data_12, cmap='Blues', origin='lower',
             aspect='auto', vmin=0, vmax=vmax)
plt.colorbar(label='Rainfall (mm)')
